...
public interface ImovelDao extends JpaRepository<Imovel, Integer> {
	...
	@Query(" select p from Imovel p where p.ativo = 1 AND CAST(p.id as string) like (%:pesq_codigo%) AND CAST(p.salas as string) like IFNULL(:pesq_salas,'%%') AND CAST(p.vagas as string) like IFNULL(:pesq_vagas,'%%') AND CAST(p.quartos as string) like IFNULL(:pesq_quartos,'%%') AND CAST(p.banheiros as string) like IFNULL(:pesq_banheiros,'%%') AND CAST(p.tamanho as string) like IFNULL(:pesq_tamanho,'%%') AND lower(p.tipoImovel.nome) like (%:pesq_tipoImovel%) AND (  lower(p.rua) like (%:pesq_localizacao%) OR lower(p.cidade) like (%:pesq_localizacao%) OR lower(p.bairro) like (%:pesq_localizacao%) OR lower(p.estado) like (%:pesq_localizacao%) OR lower(p.cep) like (%:pesq_localizacao%)  ) order by id desc")
